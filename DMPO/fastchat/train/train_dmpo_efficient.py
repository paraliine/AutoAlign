...
def trainer_save_model_safe(trainer: transformers.Trainer):
    ...
    if not isinstance(trainer.model, FSDP):
        trainer.save_model()
        return
...
def mask_labels(conversation, target, tokenizer, conv, gamma=0.7):
    if conv.sep_style == SeparatorStyle.ADD_COLON_TWO:
        ...
        return mask_chatml_labels(conversation, target, tokenizer, conv)
    else:
...
def mask_chatml_labels(conversation, target, tokenizer, conv):
    sep = conv.sep + "\n" + conv.roles[1] + "\n"
    conv.sep2 = "<|im_end|>\n<|im_start|>user"
    index = []
    ...
    for turn in turns:
        ...
        if turn.startswith("<|im_start|>system\n") and sep not in turn:
            target[cur_len : cur_len + turn_len + sep2_len] = IGNORE_TOKEN_ID
            cur_len += turn_len + sep2_len
            continue
        ...
        answer_start = cur_len + instruction_len
        answer_end = cur_len + turn_len
        if answer_start < min(answer_end, target.shape[0]):
            index.append(answer_start)
            index.append(min(answer_end, target.shape[0]))

        target[cur_len + turn_len : cur_len + turn_len + sep2_len] = IGNORE_TOKEN_ID
        cur_len += turn_len + sep2_len
    ...
    if cur_len < tokenizer.model_max_length:
        if cur_len != total_len:
            z = target.clone()
            debug_token_id = tokenizer.unk_token_id
            if debug_token_id is None:
                debug_token_id = tokenizer.eos_token_id
            z = torch.where(z == IGNORE_TOKEN_ID, debug_token_id, z)
            rank0_print(conversation)
            print("#" * 50)
            rank0_print(tokenizer.decode(z))
            target[:] = IGNORE_TOKEN_ID
...
def train():
    ...
    elif tokenizer.unk_token is not None and tokenizer.pad_token != tokenizer.unk_token:
